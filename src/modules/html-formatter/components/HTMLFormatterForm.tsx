...
interface HTMLFormatterFormProps {
  service: HTMLFormatterService;
}
...
const HTMLFormatterForm: VFC<HTMLFormatterFormProps> = ({ service }) => {
  ...
  useEffect(() => {
    ...
    setConvertedText(service.format(history[0].originalHTML));
  }, [service]);
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const convertedText = service.format(originalText, service.getOptions());
...
export default HTMLFormatterForm;
